...
ArgvParser::ArgvParser() :
		max_key(1), help_option(0) // must be smaller than max_key initially
...
string ArgvParser::optionValue(const string& _option) const {
	...
	if(key == -1) {
		cerr
				<< "ArgvParser::optionValue(): Requested value of an option the parser did not find or does not know."
				<< endl;
		return ("");
	}
			...
			else // handle short options
			{
					 ...
					 // something like '-s 4'
				{
					...
					if(argument.length() == 1) // if a single short option
						...
						if((option2attribute.find(key)->second
								& OptionRequiresValue)) {
							if(i + 1 >= _argc) // are there arguments left?
									{
								error_option = argument; // store the option that caused the error
	...
	for(it = option2attribute.begin(); it != option2attribute.end(); it++) {
		...
		if(it->second & OptionRequired) {
			...
			if(option2value.find(it->first) == option2value.end()) {
				...
				list < string > alternatives = getAllOptionAlternatives(
						it->first);
...
string ArgvParser::usageDescription(unsigned int _width) const {
	...
	for(std::map<int, std::string>::const_iterator alt =
			errorcode2descr.begin(); alt != errorcode2descr.end(); ++alt) {
		ostringstream code;
		code << alt->first;
...
bool ArgvParser::defineOption(const string & _name, const string& _descr,
		OptionAttributes _attrs) {
	// do nothing if there already is an option of this name
	if(isDefinedOption(_name)) {
		cerr
				<< "ArgvParser::defineOption(): The option label equals an already defined option."
				<< endl;
		return (false);
	}
	...
	if(_name.length() == 1 && isDigit(_name[0])) {
		cerr
				<< "ArgvParser::defineOption(): Digits as short option labels are not allowd."
				<< endl;
		return (false);
	}
...
bool ArgvParser::defineOptionAlternative(const string & _original,
	...
	if(!isDefinedOption(_original)) {
		cerr
				<< "ArgvParser::defineOptionAlternative(): Original option label is not a defined option."
				<< endl;
		return (false);
	}
	...
	if(_alternative.length() == 1 && isDigit(_alternative[0])) {
		cerr
				<< "ArgvParser::defineOptionAlternative(): Digits as short option labels are not allowd."
				<< endl;
		return (false);
	}
	...
	if(isDefinedOption(_alternative)) {
		cerr
				<< "ArgvParser::defineOptionAlternative(): The alternative option label equals an already defined option."
				<< endl;
		return (false);
	}
...
bool ArgvParser::setHelpOption(const string& _shortname,
		const string& _longname, const string& _descr) {
	// do nothing if any name is already in use
	if(isDefinedOption(_shortname) || isDefinedOption(_longname)) {
		cerr
				<< "ArgvParser::setHelpOption(): Short or long help option label equals an already defined option."
				<< endl;
		return (false);
	}
...
std::string CommandLineProcessing::formatString(const std::string& _string,
		unsigned int _width, unsigned int _indent) {
	...
	while(pos < _string.length()) {
		...
		if(!check_truncation)
			...
			string::size_type last_white_space = line.find_last_of(
					" \a\b\f\n\r\t\v");
